...
export function spawnChrome(chromePath: string, profileDir: string, port: number, url: string): ChildProcess {
  return spawn(
    chromePath,
    [`--user-data-dir=${profileDir}`, `--remote-debugging-port=${port}`, "--no-first-run", "--no-default-browser-check", url],
    { stdio: "ignore" }
  );
}
