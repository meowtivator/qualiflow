...
//   설치: %LOCALAPPDATA%\QualiFlow 로 복사 + 작업 스케줄러(로그온 시) 상주 두 개 등록:
//     - watch(실시간 fetch→클라우드 push, 인박스 최신화)
//     - serve(대시보드가 보낸 답장 발송)
//   설치 끝에 run.cmd setup(설정 마법사)을 실행 → 브라우저 자동 오픈(코드 페어링 + 채널 로그인).
...
writeFileSync(
  ...
  [
    ...
    "echo.",
    "rem 설정 마법사(브라우저 자동 오픈): 클라우드 페어링 + 채널 연결을 클릭으로. 마법사를 마치면(창 닫기) 이 창도 끝.",
    `call "%DEST%\\run.cmd" setup`,
    "pause"
  ].join(CRLF) + CRLF
);
