...
assert.ok(/Get-CimInstance[\s\S]*ExecutablePath[\s\S]*QualiFlow[\s\S]*Stop-Process/.test(bat), "복사 전에 QualiFlow 설치폴더의 node.exe 를 표적 종료해 파일락을 풀어야 한다(install/uninstall 과 동일 방식)");
