...
#   사용자가 QualiFlow 웹의 "채널 추가" 버튼을 누르면 이 로컬 마법사가 열린다(브라우저는 여기서 안 염).
...
echo "(이 창을 닫아도 백그라운드 동기화는 계속됩니다. 나중엔 QualiFlow 웹의 '채널 추가' 버튼으로도 열려요.)"
echo ""
...
3) 마법사에서:
     ① QualiFlow 웹의 "연결 소스 → 코드 발급"에서 코드를 받아 붙여넣기 → 페어링
     ② 연결할 채널 버튼을 눌러 로그인(알리바바=로그인창 / WhatsApp=QR / 텔레그램=전화코드)
4) 끝. 이후로는 백그라운드에서 자동으로 실시간 동기화 + 발송이 됩니다(로그인 시 자동 시작).
   ★ 나중에 채널을 더 추가할 때는, 브라우저를 켤 필요 없이 QualiFlow 웹의 "연결 소스 → 채널 추가"
