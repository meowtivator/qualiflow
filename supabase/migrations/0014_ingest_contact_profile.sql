-- ingest_conversations에 '연락처 프로필 강화'를 추가한다(알리바바 enrichment용).
-- ★0012(author+attachments) 위에 쌓는다 — CREATE OR REPLACE라 함수 전체를 포함하되, 변경점은:
--   (1) lead 생성 시 contact.companyName/countryCode/profileImageUrl 을 leads에 채움.
--   (2) 기존 lead도 그 값이 새로 들어오면 채움(없으면 기존 유지 — coalesce).
--   (3) channel_identity 생성 시 profile_image_url 채움.
--   값이 없으면(텍스트 채널 등) 모두 그대로 동작 — 하위호환.
-- 입력 contact: { id, name?, handle?, companyName?, countryCode?, profileImageUrl? }
