-- messages.author 계약 채우기: ingest가 메시지를 저장할 때 core의 MessageAuthor({ role, displayName }) 모양을 쓴다.
-- ★변경점은 단 하나 — 아래 ingest_conversations 안 "3c) 메시지" insert에 author 컬럼을 추가한 것뿐이다.
--   토큰→워크스페이스 도출(스푸핑 차단), SECURITY DEFINER, 멱등(ON CONFLICT) 로직은 0009와 동일하게 유지.
-- ★비즈니스 규칙(신규): author.role/displayName 도출
--   - inbound(상대가 보냄)  → { role:'lead',     displayName: 대화 contact.name (없으면 '(이름 없음)') }
--   - outbound(우리가 보냄) → { role:'operator' }  (인그est 입력에 운영자 이름이 없어 displayName 생략 → reader가 폴백)
--   기존 데이터(0011 이전에 저장된 메시지)는 author='{}' 그대로 → reader가 방향 기반 기본값으로 폴백(깨지지 않음).
  ...
  loop
    ...
    loop
      ...
      insert into public.messages (
        workspace_id, thread_id, lead_id, channel_id, external_message_id, direction, author, content, sent_at
      )
      values (
        v_workspace_id, v_thread_id, v_lead_id, p_channel,
        v_msg->>'id',
        case when v_msg->>'direction' = 'outbound' then 'outbound' else 'inbound' end,
        -- ★추가: author(core MessageAuthor). inbound=상대(lead) 이름 / outbound=운영자(이름 없음→reader 폴백).
        case
