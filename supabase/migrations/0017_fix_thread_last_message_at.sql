-- 근본 수정: threads.last_message_at 이 "마지막 메시지 시각"이 아니라 ingest 시각(now())으로
-- 박히던 버그를 고친다. (0014의 ingest_conversations line 154: last_message_at = now())
--   (1) 함수 수정: 메시지 적재 후 last_message_at 을 그 스레드의 실제 max(sent_at) 으로 설정.
--   (2) 백필: 기존 모든 thread 의 last_message_at 을 실제 max(sent_at) 으로 교정.
-- 그러면 메신저 좌측 날짜·정렬, ① 반응 바이어 정렬, 바이어 여정 "마지막 연락" 이 전부 맞게 된다.
-- ★0014 함수 전체를 그대로 복제하되 마지막 update 한 줄만 바꾼다(CREATE OR REPLACE).
...
begin
  ...
  loop
    ...
    if v_lead_id is null then
      insert into public.leads (workspace_id, display_name, company_name, country_code, profile_image_url, source_channel_ids)
      values (
        v_workspace_id,
...
-- 백필: 기존 모든 thread 의 last_message_at 을 실제 max(sent_at) 으로 교정.
update public.threads t set
  last_message_at = sub.max_sent,
  updated_at = now()
from (
...
where sub.thread_id = t.id
  and t.last_message_at is distinct from sub.max_sent;
